...
//                .leftJoin(member.team, team).on(team.name.eq("teamA")) //outer join경우 on절을 사용할 수 바께 없다.
    ...
    /**
     * BooleanBuilder로 동적쿼리 사용하기
     * 코드를 보기가 어렵다. (가독성이 나쁨)
     */
    @Test
    void dynamicQuery_BooleanBuilder() {
        String usernameParam = "member1";
        Integer ageParam = null; //age가 null이면 username만 들어가게 된다.
//        Integer ageParam = 10;
    ...
    private List<Member> searchMember2(String usernameCond, Integer ageCond) {
        return queryFactory
                .selectFrom(member)
                .where(usernameEq(usernameCond), ageEq(ageCond)) //where에 null이 들어가면 무시가 된다., 메서드를 다른 코드에서 재활용을 할 수 있다.
